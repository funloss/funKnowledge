        ...
        if content.startswith('---'):
            ...
            if end_match:
                ...
                if 'cover:' in properties:
                    print(f"已存在cover标签: {file_path}")
                    return
                
                # 在douban_link下面添加cover标签
                updated_properties = re.sub(r'(douban_link:.*?)\n', '\\1\ncover: \n', properties)
                
                # 如果没有找到douban_link，就在tags下面添加
                if updated_properties == properties:
                    updated_properties = re.sub(r'(tags:.*?)\n', '\\1\ncover: \n', properties)
                    
                # 如果仍然没有更新，说明属性块格式不同，直接在属性块末尾添加
                if updated_properties == properties:
                    # 确保正确处理换行符
                    cover_line = 'cover: \n'
                    updated_properties = properties.replace('---', cover_line + '---')
